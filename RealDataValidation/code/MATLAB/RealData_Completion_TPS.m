...
for tt = 1:T_test
    ...
    for kk = 1:K
        Xtps{tt}(:,kk) = TPS(x_IND,y_IND,Xomega(:,kk),completion_grid_x,completion_grid_y,1e-6,0);
    end
    
    %%
    % NMSE of completion
    NMSEcomp(tt) = frob(Xtps{tt} - Xtrue_tt_mat)^2/frob(Xtrue_tt_mat)^2;
    
end
figure;
plot(NMSEcomp,'LineWidth',1.25,'DisplayName',['completion via TPS']);
grid on;
...
%% save NMSEmat
save(['..\..\result\NMSE_TPS_rho',num2str(rho),'.mat'],"NMSEcomp");