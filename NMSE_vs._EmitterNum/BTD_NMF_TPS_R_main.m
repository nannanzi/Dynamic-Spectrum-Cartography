...
Rall = 2:2:6;
NMSE_CBTD = zeros(length(Rall),T);
NMSE_NMF = zeros(length(Rall),T);
NMSE_TPS = zeros(length(Rall),T);
...
%initializing cache variables for OptDSC
    ...
    %% baseline 1: LL1-TPS
        ...
        NMSE_CBTD(rnum,tt) = frob(Xhat_ll1_tt - Xcubet).^2/frob(Xcubet).^2;
    %% baseline 2: NMF-TPS
        ...
        NMSE_NMF(rnum,tt) = frob(Xhat_nmf_tt - Xcubet).^2/frob(Xcubet).^2;
    ...
    %% baseline 3: Interpolation
        ...
        NMSE_TPS(rnum,tt) = frob(Xhat_tps_tt - Xcubet).^2/frob(Xcubet).^2;
