...
%         [Ginit_opt_tt, Cinit_opt_tt] = NMF_HALS(Ytt,Rest,50,Ginit_opt_tt,Cinit_opt_tt); %fine-tunned via NMF_HALS
    Cinit_opt{tt} = Cinit_opt_tt;

    if tt > 1 %unifying ambiguities
        Cinit_opt_historical = Cinit_opt{tt-1};
        [err,permutation,scaling,Cinit_opt_tt] = cpderr(Cinit_opt_historical,Cinit_opt_tt);
        Cinit_opt{tt} =  Cinit_opt_tt;
%             Cinit_opt{tt} = Cinit_opt_historical;
     
        Ginit_opt_tt = Ytt/(Cinit_opt{tt}');
%             Ginit_opt_tt = Ginit_opt_tt
    end
    ...
    for rr = 1:Rest
        ...
        [A{rr},B{rr},Rcache{rr},scache{rr},Pcache{rr},qcache{rr},c{rr}] = UpdateAll_Grad(A{rr},B{rr},c{rr},Rcache{rr},scache{rr},Pcache{rr},qcache{rr},Gmat{rr},Wmatt,lambda);   
    ...
    if tt == vis_time
        ...
        title(['IncreDSC,NMSE=', num2str(10*log10(NMSE_IncreDSC(tt)),'%3.3f')])
