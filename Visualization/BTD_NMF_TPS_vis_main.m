...
%% baseline 3: Interpolation
    ...
    if tt == check_timeslot
        ...
        Xhat_ll1_tt(Xhat_ll1_tt<valmin) = valmin;
        Xhat_ll1_tt(Xhat_ll1_tt>valmax) = valmax;
        Xslice = squeeze(Xhat_ll1_tt(:,:,check_frequencybin));
        contourf(10*log10(Xslice),100,'linecolor','None');
        colormap jet;
        set(gca,'xtick',[],'xticklabel',[])
        set(gca,'ytick',[],'yticklabel',[])
        title(['BTD-TPS,NMSE=', num2str(NMSE_CBTD(tt),'%3.3f')])
        ...
        Xhat_nmf_tt(Xhat_nmf_tt<valmin) = valmin;
        Xhat_nmf_tt(Xhat_nmf_tt>valmax) = valmax;
        Xslice = squeeze(Xhat_nmf_tt(:,:,check_frequencybin));
        contourf(10*log10(Xslice),100,'linecolor','None');
        colormap jet;
        set(gca,'xtick',[],'xticklabel',[])
        set(gca,'ytick',[],'yticklabel',[])
        title(['NMF-TPS,NMSE=', num2str(NMSE_NMF(tt),'%3.3f')])
        set(gca,'FontName','Times New Roman','FontSize',15,'LineWid',1);
        axes('position',[0.2,0.02,.6,.3])
        axis off
        my_handle = colorbar('east');
        ...
        Xhat_tps_tt(Xhat_tps_tt<valmin) = valmin;
        Xhat_tps_tt(Xhat_tps_tt>valmax) = valmax;
        Xslice = squeeze(Xhat_tps_tt(:,:,check_frequencybin));
        contourf(10*log10(Xslice),100,'linecolor','None');
        colormap jet;
        set(gca,'xtick',[],'xticklabel',[])
        set(gca,'ytick',[],'yticklabel',[])
        title(['TPS,NMSE=', num2str(NMSE_TPS(tt),'%3.3f')])
        set(gca,'FontName','Times New Roman','FontSize',15,'LineWid',1);
        axes('position',[0.2,0.02,.6,.3])
        axis off
        my_handle = colorbar('east');
