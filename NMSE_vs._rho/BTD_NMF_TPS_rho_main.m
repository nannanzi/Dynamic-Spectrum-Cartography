...
NMSE_CBTD = zeros(length(p),T);
NMSE_NMF = zeros(length(p),T);
NMSE_TPS = zeros(length(p),T);
for pp = 1:length(p)
    rho = p(pp);
    for tt = 1:T
    ...
    %% baseline 1: LL1-TPS
        ...
        NMSE_CBTD(pp,tt) = frob(Xhat_ll1_tt - Xcubet).^2/frob(Xcubet).^2;
    %% baseline 2: NMF-TPS
        ...
        NMSE_NMF(pp,tt) = frob(Xhat_nmf_tt - Xcubet).^2/frob(Xcubet).^2;
    ...
    %% baseline 3: Interpolation
        ...
        NMSE_TPS(pp,tt) = frob(Xhat_tps_tt - Xcubet).^2/frob(Xcubet).^2;
